...
/**
 * Handles all interaction with the User from the text-based interface.&nbsp;
 * Consists of a single public-facing static start(), which enforces Singleton properties for input.&nbsp;
 * 
...
public class InputHandler {
    ...
    /**
     * Starts the input loop.&nbsp; The loop will continue until "quit" or "q" is recieved.
     */
    ...
    //TODO IMPLEMENT A execute(String input) for a simulated command line window
    ...
    private void playerSummary(String string) throws Exception{
            Player temp = Player.findPlayer(string);
    ...
    private void listGames(String[] input){
            if(GameStat.getGames() == null || GameStat.getGames().isEmpty())
                    ...
                    Player temp = Player.getTracked(summoner);
                    if(temp != null){
                            games = temp.getGames();
                    }
                    else{
